...
def _full_sections(results: dict[str, float]) -> list[tuple[str, list]]:
    ...
    return [
        (title, sorted(buckets[title], key=lambda r: r[0]))
        for title in ("gloom (fetch)", "assign", "delete")
    ]
...
def _make_chart(sections: list[tuple[str, list]], out_path: str) -> None:
    ...
    for ax, (title, rows) in zip(axes, sections):
        ...
        for i, (gt, gv) in enumerate(zip(g_times, glomt)):
            ax.text(max(gt, gv) * 1.04, i, f"{gv / gt:.1f}×",
...
def main() -> None:
    ...
    parser.add_argument("--full",     action="store_true", help="Include every gloomy/glom pair, not just the curated summary")
    parser.add_argument("--no-chart", action="store_true", help="Skip chart generation (matplotlib not required)")
    parser.add_argument("--chart-out", default=str(repo_root / "bench.png"), metavar="FILE",
